...
$sql = "SELECT * FROM outsource_plan WHERE type = '$type' AND id != 5 ORDER BY price ASC";
$db->sql($sql);
$res = $db->getResult();
$num = $db->numRows($res);
...
if ($num >= 1) {
    foreach ($res as $row) {
        ...
        $plan_id = $row['id'];
        $sql_check_plan_1_2_4_6 = "SELECT * FROM outsource_user_plan WHERE user_id = $user_id AND (plan_id = 1 OR plan_id = 2 OR plan_id = 4 OR plan_id = 6)";
